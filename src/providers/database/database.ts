...
export class DatabaseProvider {
  ...
  renderProperties() : Observable<any>
  {

     return new Observable(observer =>
     {
        let property : any = [];
        firebase.database().ref('PROPERTY').orderByKey().once('value', (items : any) =>
        {
           items.forEach((item) =>
           {
              property.push({
  ...
  uploadPropertyImage(imageString) : Promise<any>
  {
     let image       : string  = 'property-' + new Date().getTime() + '.jpg',
         storageRef  : any,
         parseUpload : any;

     return new Promise((resolve, reject) =>
     {
        storageRef       = firebase.storage().ref('propertyIMG/' + image);
        parseUpload      = storageRef.putString(imageString, 'data_url');
  ...
  addPropertyToDatabase(propertyObj) : Promise<any>
  {
     return new Promise((resolve) =>
     {
        let addRef = firebase.database().ref('PROPERTY');
        addRef.push(propertyObj);
        resolve(true);
     });
  }

  updatePropertyDatabase(id, propertyObj) : Promise<any>
  {
     return new Promise((resolve) =>
     {
        var updateRef = firebase.database().ref('PROPERTY').child(id);
       updateRef.update(propertyObj);
        resolve(true);
     });
  }
  ...
  deleteProperty(id) : Promise<any>
  {
     return new Promise((resolve) =>
     {
        let ref = firebase.database().ref('PROPERTY').child(id);
        ref.remove();
        resolve(true);
     });
  }
